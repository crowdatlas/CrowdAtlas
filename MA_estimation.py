...
def MA_approach(date, station, hour):
    ...
    for d in ref_dates:
        xs, ys = data_inference(d, station, hour)
    ...
    xs_test, ys_test = data_inference(date, station, hour)
    ...
    df_group = df_record.groupby(by=['start_station', 'start_time', 'end_time']).sum().reset_index()
    ...
    if not os.path.exists(dir):
        os.mkdir(dir)
    df_record_opt.to_csv(dir + '/' + station + '_num_dist_bsl.csv', index=None)
...
if __name__ == '__main__':
    ...
    for k in range(52):
        ...
        for q in range(0,52):
            df = pd.read_csv('sta_results/' + date + '/' + str(start_hour -1) + '/' + station_list[q] + '_num_dist_bsl.csv')
            for h in range(start_hour, start_hour + duration):
                MA_approach(date,station_list[q],h)
                df_temp = pd.read_csv('sta_results/' + date + '/' + str(h) + '/' + station_list[q] + '_num_dist_bsl.csv')
        ...
        if not os.path.exists(dir + '/' + date):
            os.mkdir(dir + '/' + date)
        df_record.to_csv(dir + '/' + date + '/' + station_list[k] + '_num_comp_bsl.csv', index=None)

    df_mape.to_csv(dir + '/' + date + '/MAPE_across_time_station_bsl.csv', index=None)
    ...
    for i in range(duration * 12):
        ...
        for k in range(0, len(stations)):
            df = pd.read_csv(dir + '/' + date + '/' + station_list[k] + '_num_comp_bsl.csv')
            record += [int(df.iloc[i, 1]), int(df.iloc[i, 2])]

        df_merge.loc[i] = record

    df_merge.to_csv(dir + '/' + date + '/merged_arv_comp_bsl.csv', index=None)